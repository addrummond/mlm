...
touch_position get_touch_position(uint32_t chan0, uint32_t chan1, uint32_t chan2)
{
    ...
    if (rat2 < THRESHOLD_FRAC * rat2nopress / 256)
        return CENTER_BUTTON;
    if (rat0 < LEFT_RIGHT_THRESHOLD_FRAC * rat0nopress / 256 && rat3 < THRESHOLD_FRAC * rat3nopress / 256)
        return RIGHT_BUTTON;
    if (rat1 < LEFT_RIGHT_THRESHOLD_FRAC * rat1nopress / 256 && rat4 < THRESHOLD_FRAC * rat4nopress / 256)
        return LEFT_BUTTON;
...
{
    LESENSE_ScanFreqSet(scan_freq, SLOW_PAD_SCAN_HZ);
}
